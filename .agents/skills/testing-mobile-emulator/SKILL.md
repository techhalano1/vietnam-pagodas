...
- System theme check: `adb shell cmd uimode night yes|no` toggles OS dark mode live (`settings get secure ui_night_mode` → 2 = dark). To observe the app's *default* theme (no stored `vp-theme`), clear Expo Go data first: `adb shell pm clear host.exp.exponent` — this also drops `adb reverse`, re-add it before relaunching.
- Fake the date (e.g. to hit a lunar holiday on Home): the emulator re-syncs the clock within ~20 s unless `adb shell settings put global auto_time 0` is set first; then `adb shell date MMDDhhmmYYYY.ss` (device is rooted). Moving the clock *backwards* makes Expo Go fail with "Something went wrong" (expo-updates SQLite UNIQUE constraint on scope_key/commit_time) — fix with `pm clear host.exp.exponent` + relaunch. Restore afterwards: `auto_time 1` + `adb shell date "$(date -u +%m%d%H%M%Y.%S)"`. Useful lunar dates: solar 2026-04-06 = 19/2 Bính Ngọ → "Vía Quan Thế Âm Bồ Tát (đản sinh)" (compute others with `solarToLunar`/`holidaysOn` in `mobile/src/lib/lunar.ts`).
- After `am force-stop` + `am start -a VIEW -d exp://localhost:8081`, Expo Go sometimes stays in the background (launcher visible); fire the intent a second time. The first launch after `pm clear` shows the Expo dev-menu overlay — tap "Continue"/× before asserting on the screen.
