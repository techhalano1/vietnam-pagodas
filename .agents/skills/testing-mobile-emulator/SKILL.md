...
- Layout shifts: when the red offline caption is visible the PlayerBar buttons sit ~20 px higher; the Expo Go toast's × overlaps the PlayerBar × — if the toast fades right before your tap you stop the track instead.
- Setting repeat after a track has already ended shows `Biến 1/3` with ▶ but does not auto-restart; press ▶. Pressing × exactly at a round boundary can be undone by the auto-repeat restart.
