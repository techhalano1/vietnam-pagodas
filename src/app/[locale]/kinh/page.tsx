...
const kindBadge: Record<string, string> = {
  ...
  mantra: "bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200",
...
export default function ScripturesPage({ params }: { params: { locale: string } }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <h1 className="text-3xl font-semibold tracking-tight">📖 {t.scripturesTitle}</h1>
      <p className="mt-1 text-stone-500 dark:text-stone-400">{t.scripturesIntro}</p>
                ...
                {items.map((s) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      <span className={`rounded-full px-2 py-0.5 font-medium ${kindBadge[s.kind]}`}>
                        {t.scriptureKind[s.kind]}
                      </span>
                      <span className="text-stone-400">{t.versesCount(s.verses.length)}</span>
                      {hasHanViet(s) && (
